...
export class GameScene extends Phaser.Scene {
  ...
  init(data: { chapterId?: number }) { this.chapterId = data.chapterId || 1; this.enemies = []; this.bossSpawned = false; this.killCount = 0; this.spawnTimer = 0; }
  create() {
    const { width, height } = this.cameras.main;
    this.worldW = width*3; this.worldH = height*3;
    ...
    this.setupInput();
    this.setupAmbient();
    this.fpsText = this.add.text(10, 10, '', { fontSize:'10px', fontFamily:'monospace', color:'#00ff41' }).setScrollFactor(0).setDepth(100);
    this.setupEventListeners();
    this.spawnWave();
    useGameStore.getState().emitGameEvent('scene-ready', { scene: 'GameScene' });
  }
  ...
  private createPlayer() {
    ...
    this.player = new PlayerEntity(this, this.worldW/2, this.worldH/2, store.selectedCharacter || 'javascript-fox');
  }
  private setupInput() {
    ...
    this.skillKeys = { Q:this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.Q), W:this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.W), E:this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.E), R:this.input.keyboard!.addKey(Phaser.Input.Keyboard.KeyCodes.R) };
  ...
  private setupAmbient() {
    ...
    this.add.particles(this.worldW/2, this.worldH/2, 'particle_corruption', { speed:{min:5,max:20}, scale:{start:0.5,end:0}, alpha:{start:0.3,end:0}, lifespan:3000, frequency:200, quantity:2, tint:colors[this.chapterId]||0x222222, blendMode:'ADD' }).setDepth(-1);
  }
  private setupEventListeners() {
    const store = useGameStore.getState();
    this.unsubs.push(
      store.onGameEvent('skill-used', (data: unknown) => { const { key } = data as { key: SkillKey }; this.player.executeSkill(key); }),
      store.onGameEvent('item-used', (data: unknown) => { const { item } = data as { item: { itemId: string } }; if (item.itemId === 'health_potion') { this.add.particles(this.player.sprite.x, this.player.sprite.y, 'particle_heal', { speed:{min:30,max:80}, scale:{start:0.6,end:0}, lifespan:1000, quantity:20, tint:0x00ff88, blendMode:'ADD', gravityY:-50 }); } })
    );
  }
  private async spawnWave() {
    if (this.enemies.length >= this.maxEnemies) return;
    const count = Math.min(3, this.maxEnemies-this.enemies.length);
    for (let i=0; i<count; i++) await this.spawnEnemy(false);
  }
  private async spawnEnemy(isBoss: boolean) {
    let x: number, y: number;
    do { x=64+Math.random()*(this.worldW-128); y=64+Math.random()*(this.worldH-128); }
    while (this.player && Phaser.Math.Distance.Between(x,y,this.player.sprite.x,this.player.sprite.y)<300);
    try {
      ...
      const p = this.add.particles(x, y, 'particle_corruption', { speed:{min:50,max:150}, scale:{start:0.8,end:0}, lifespan:600, quantity:15, tint:beast.tintColor, blendMode:'ADD' });
  ...
  update(time: number, delta: number) {
    ...
    const store = useGameStore.getState();
    if (store.settings.showFPS) this.fpsText.setText(`FPS: ${Math.round(1000/delta)}`).setVisible(true);
    else this.fpsText.setVisible(false);
    this.player.update(delta, this.cursors, this.wasd, this.skillKeys);
    this.enemies = this.enemies.filter(e => { if (!e.isAlive()) { this.handleDeath(e); return false; } e.update(delta, this.player); return true; });
  ...
  private handleDeath(enemy: EnemyEntity) {
    ...
    beast.dropTable.forEach(drop => {
      if (Math.random()*100 < drop.dropChance) {
        ...
        store.addItem({ id:`item_${Date.now()}_${Math.random()}`, itemId:drop.itemId, name:drop.name, description:`Dropped by ${beast.corruptedName}`, type:drop.itemId==='health_potion'?'consumable':'material', rarity:beast.isBoss?'rare':'common', quantity:qty, icon:this.getItemIcon(drop.itemId) });
      }
    });
    const p = this.add.particles(enemy.sprite.x, enemy.sprite.y, 'particle_corruption', { speed:{min:80,max:200}, scale:{start:1,end:0}, lifespan:800, quantity:25, tint:beast.tintColor, blendMode:'ADD' });
    ...
    this.killCount++;
    if (beast.isBoss) { store.defeatBoss(beast.pokemonId); store.addNotification({ type:'success', title:'🏆 BOSS DEFEATED!', message:`${beast.corruptedName} purified!`, duration:5000, icon:'🏆' }); this.time.delayedCall(3000, () => this.completeChapter()); }
  }
  ...
  private checkCollisions() {
    ...
    this.enemies.forEach(enemy => {
      if (!enemy.isAlive()||!enemy.sprite.active) return;
      ...
      this.player.skillHitboxes.forEach(hb => {
        if (!hb.active) return;
        const hbb = hb.getBounds();
        if (Phaser.Geom.Intersects.RectangleToRectangle(hbb, eb)) {
          const hitSet = hb.getData('hitEnemies')||new Set();
          if (!hitSet.has(enemy.id)) {
            hitSet.add(enemy.id); hb.setData('hitEnemies', hitSet);
  ...
  showDmgNum(x: number, y: number, dmg: number, color='#ffffff', isCrit=false) {
    ...
    const t = this.add.text(x, y, isCrit?`${dmg}!`:`${dmg}`, { fontSize:isCrit?'18px':'14px', fontFamily:'monospace', color, stroke:'#000000', strokeThickness:2 }).setDepth(50);
    this.tweens.add({ targets:t, y:y-60, alpha:0, duration:1200, ease:'Power2', onComplete:()=>t.destroy() });
  }
  private togglePause() { this.isPaused = !this.isPaused; this.isPaused?this.physics.pause():this.physics.resume(); useGameStore.getState().emitGameEvent('game-paused', { paused:this.isPaused }); }
  private getItemIcon(itemId: string): string { const icons: Record<string,string> = { code_fragment:'📄', data_crystal:'💎', corrupted_core:'🔴', legendary_shard:'⭐', system_key:'🔑', health_potion:'🧪' }; return icons[itemId]||'📦'; }
  shutdown() { this.unsubs.forEach(u=>u()); this.enemies.forEach(e=>e.destroy()); this.player?.destroy(); }
}
